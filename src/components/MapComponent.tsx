...
interface MapComponentProps {
  ...
  onSiteClick?: ((site: HeritageSite) => void) | null;
  customSites?: HeritageSite[] | null;
}

// Dynamic Heritage Icons - Gold for discovered, Red for undiscovered
const getCustomIcon = (isUnlocked: boolean) => {
...
const MapComponent: React.FC<MapComponentProps> = ({ 
  ...
  const [narratingSites, setNarratingSites] = useState(new Set<number>());
  const [unlockedHistory, setUnlockedHistory] = useState<number[]>([]);
  const [heritageUnlocked, setHeritageUnlocked] = useState<number[]>([]);
  
  const kolkataPosition = center;

  const handleNarrate = async (siteName: string, siteId: number) => {
  ...
  const handleStoryClick = async (site: HeritageSite) => {
    try {
      const response = await fetch('http://localhost:5678/webhook-test/narrate-heritage', {
        method: 'POST',
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-screen bg-cream">
        <div className="text-heritage-red text-xl font-display">Discovering Kolkata's Stories...</div>
      </div>
    );
  }
        ...
        >
          <Popup>
            <div className="text-center p-2">
              <h3 className="font-bold text-heritage-red text-lg font-display">{site.name}</h3>
              <p className="text-xs text-muted-foreground italic mb-2">Heritage Hub</p>
              <button 
                className="bg-heritage-red text-cream px-3 py-1 rounded-full text-xs hover:opacity-80 transition-all"
                onClick={() => handleStoryClick(site)}
              >
                Hear the Story
              </button>
            </div>
